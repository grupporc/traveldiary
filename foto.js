...
function aggiungiAlbum(a_t,titolo){
...
function creaFoto(a_t, id_client, foto, id_album, titolo){
    return new Promise((resolve,reject) =>{
        console.log("CREA FOTO!"+id_album+" "+foto+" "+titolo);
        var fotobin = Buffer.from(fs.readFileSync("fbimages/"+id_client+"/"+titolo+"/"+foto, 'binary'), 'binary');
...
function caricaFoto(a_t,id_client){
            ...
            function(resp){
                ...
                async function process(){
                    for(var j=0; j<num_photo; j++){
                        ...
                        await creaFoto(a_t,id_client, photo, id_album, title).then(
                            function(resp){
                                console.log(resp);
                            }
                        ).catch( 
